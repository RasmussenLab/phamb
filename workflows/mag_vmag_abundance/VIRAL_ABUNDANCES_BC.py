...
def get_bin_abundances(args,sample_list, breadth_of_coverage, contig_order):
    ...
    #viralbins_table = '07_binannotation/checkv/VAMB_bins/nc_viralbins.txt'
    viral_bins = set()

    ### Parse all Bins with a DGR casette
    for f in ['07_binannotation/checkv/VAMB_bins/proteome_annotation/nc.bins.interpros.txt','07_binannotation/checkv/VAMB_bins/proteome_annotation/MQLQND.bins.interpros.txt']:
        with open(f,'r') as infile:
            ...
            for line in infile:
                ...
                DGR = line[-1]
                if DGR != 'NA':
                    viral_bins.add(binid)

    ### Read in all Viral bins tested by CheckV
    #with open(viralbins_table,'r') as infile:
    #    for line in infile:
     #       binid = line.strip()
     #       viral_bins.add(binid)
    
    ### Include other weird stuff 
    viralbins_table = '07_binannotation/checkv/VAMB_bins/quality_summary.tsv'
    with open(viralbins_table,'r') as infile:
        infile.readline()
        for line in infile:
            line = line.strip().split('\t')
...
def write_matrices(args ,samples , motherbins_avgRPKM, motherbins_avgRPM):
    ...
    header = '\t'.join(header)
    fileout1 = os.path.join(outbase,'VAMBV3.LQHQDGR.Viralmotherbin.breadthcov.RPKM.txt')
    np.savetxt(fileout1, rpkmmatrix , delimiter='\t', header=header, newline='\n', fmt='%s')

    rpmmatrix = return_RPKM_matrix(motherbins_avgRPM, mbin_median)
    header = ['mothercluster'] + list(samples[0][1:])
    header = '\t'.join(header)
    fileout1 = os.path.join(outbase,'VAMBV3.LQHQDGR.Viralmotherbin.breadthcov.RPM.txt')
