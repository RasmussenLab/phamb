...
def load_testset():
    ...
    testset = dict()
    completeness_file = '07_binannotation/checkv/VAMB_bins/VAMB_completeness.tsv'
    with open(completeness_file,'r') as infile:
        ...
        for line in infile:
            ...
            binid = line[0]
            if line[completeness_index] == 'NA':
                line[completeness_index] = '0'
            testset[binid] = {'completeness':line[completeness_index],
            'aai_conf':line[aai_conf_index]}
    
    testbins = set()
    for binid in testset:
        if float(testset[binid]['completeness']) >= 90 and testset[binid]['aai_conf'] == 'high':
            testbins.add(binid)
    return testbins
...
def write_completeness_file(cluster_splits,contigs,hmm_completeness,out):
...
if __name__ == "__main__":
    ...
    fileout = os.path.join(args.v,'VAMB_completeness_all.tsv')
    with open(fileout,'w') as out:
        ...
        'hmm_upper_completeness','contigs_in_genome']
        out.write('\t'.join(header)+'\n')

        for binid in list(VAMB_bins):

            VAMB_bin = VAMB_bins[binid]
            contigs = VAMB_bins[binid].contigs
            cluster_splits,hmm_completeness = evaluate_VAMB_cluster( VAMB_bin, checkv_results ,refs,hmms)
            
            write_completeness_file(cluster_splits,contigs,hmm_completeness,out)